...
fn main() -> Result<()> {
    ...
    for mut detection in detections {
        ...
        let colour = classify_detection(&image_path, &detection.bbox_pixels)
...
fn classify_detection(image_path: &Path, bbox: &BBox) -> Result<&'static str> {
    ...
    for y in (top as u32..bottom as u32).step_by(stride as usize) {
        for x in (left as u32..right as u32).step_by(stride as usize) {
            ...
            if !(0.06..=0.94).contains(&nx) || !(0.12..=0.82).contains(&ny) {
                continue;
            }
            ...
            let center_weight = gaussian(nx, 0.5, 0.36) * gaussian(ny, 0.43, 0.28);
            let mut weight = 0.25 + center_weight;
